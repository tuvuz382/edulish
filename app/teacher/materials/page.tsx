  ...
  return (
    <div className="space-y-6 p-4 lg:p-6">
      <div className="flex flex-wrap items-center justify-between gap-4 animate-fade-in-up">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Tài liệu tham khảo 📚</h2>
          <p className="mt-1 text-slate-500">Truy cập nhanh các tài liệu để chia sẻ cho học sinh</p>
        </div>
      </div>
          ...
          {MATERIAL_TYPES.map((t) => (
            <button key={t.id} onClick={() => setTypeFilter(t.id)}
              className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition-all ${
                typeFilter === t.id ? "bg-emerald-600 text-white" : "bg-white border border-slate-200 text-slate-600 hover:bg-slate-50"
              }`}>
              {t.icon} {t.label}
            </button>
          ))}
              ...
              return (
                <div key={m.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm flex flex-col hover:shadow-md transition-shadow">
                  ...
                  <div className="mt-auto pt-4 flex items-center justify-between border-t border-slate-100">
                    ...
                    <a href={m.url} target="_blank" rel="noopener noreferrer" className="rounded-xl bg-emerald-100 text-emerald-700 px-4 py-1.5 hover:bg-emerald-200 text-sm font-semibold transition-colors">Truy cập</a>
                  </div>
                </div>
              );
            })}
